...
export function useBookReader(_id: string | undefined) {
  ...
  const updates: Partial<Book> = useMemo(() => ({ currentLine, lastCompleted, bookmarks }), [currentLine, lastCompleted, bookmarks]);
  const canUpdate = !loading && !loadingMore && JSON.stringify(updates) !== JSON.stringify({ currentLine: book?.currentLine, lastCompleted: book?.lastCompleted, bookmarks: book?.bookmarks });

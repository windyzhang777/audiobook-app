...
export const BookLine = React.forwardRef<HTMLLIElement, BookLineProps>(({ index, line }: BookLineProps, ref) => {
  const { currentLine, book, bookmarks, toggleBookmark, highlights, deleteLine } = useBookContext();
    ...
    >
      ...
      {readingMode === 'edit' ? (
        ...
        FEATURES.ENABLE_BOOKMARK_EDIT && (
          <Button
            key={`line-${index}-toggle-bookmark`}
            size="icon"
            variant="ghost"
            tabIndex={index === currentLine ? 0 : -1}
